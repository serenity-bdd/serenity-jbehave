...
public class JUnitScenarioReporter extends NullStoryReporter {
	...
	private Description currentScenarioTitleOrElse(TestState testState, String defaultValue) {
		return Optional.ofNullable(testState.currentScenario)
				.orElse(Description.createTestDescription(testState.currentStoryDescription.getClassName(),
						defaultValue));
	...
	public void beforeScenario(String title) {
		...
		if (!testState.isGivenStoryRunning()) {
			notifier.fireTestStarted(currentScenarioTitleOrElse(testState, title));
	...
	public void failed(String step, Throwable e) {
		...
		if (!testState.isGivenStoryRunning()) {
			...
			if (thrownException instanceof BeforeOrAfterFailed) {
				notifier.fireTestStarted(currentScenarioTitleOrElse(testState, step));
			}
			notifier.fireTestFailure(new Failure(currentScenarioTitleOrElse(testState, step), thrownException));
			notifier.fireTestFinished(currentScenarioTitleOrElse(testState, step));
			testState.failedSteps.add(currentScenarioTitleOrElse(testState, step));
			prepareNextStep();
		}
	}
